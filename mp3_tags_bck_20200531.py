...
class Mp3Tags:
    ...
    def __init__(self):
        self.filepath = ""
        self.id3_object = ""
        self.dict_id3_fields_selected = ""
        self.self.available_specified_tags = ""
        self.clean_tags = ""
    
    
    def read_all_tags_from_file(self):
    ...
    def look_for_available_specified_tags(self):
        # Checking the number of desired keys present in the id3 object
        available_specified_tags = list(self.id3_object.keys() & self.dict_id3_fields_selected.keys())
    ...
    def extract_available_specified_tags(self):

        # Storing the extracted id3 information in a dictionary.
        extracted_tags = {}  # Initialize / reset to an empty dictionary.

        for tag in self.available_specified_tags:
            id3_field = None # Initialize / reset to an empty variable.

            # Read the tags
            id3_field = self.id3_object.get(tag)  # Fetching the information for the respective id3 field from the id3 tag. Get returns a string (or None) while getall returns a list!
    ...
    def write_tags_into_id3_object(self):

        for key, value in self.clean_tags.items():

            if key == "POPM:no@email": # Does not have a text field and thus a bit different structure.
                exec(f'self.id3_object.add({value})') # Executes the following string as Python command. The f in the beginning marks this as "f string". See https://www.python.org/dev/peps/pep-0498/
                ...
                exec(f'self.id3_object.add({key}(encoding = 3, text = "{value}"))') 
