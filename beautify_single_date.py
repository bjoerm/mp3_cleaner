...
class DateBeautifier:
    ...
    def extract_year(string: str) -> str:
        ...
        if output is None:
            ...
            output is not None
        ):  # Dealing with the special of None - which is not converted into a string.
            output = str(output)

        try:
            output = re.search(r"(19|20|21)\d{2}", output).group(
                0
            )  # Look for 19xx, 20xx or 21xx (where xx are two digits). First found entry will be taken.
        except:
