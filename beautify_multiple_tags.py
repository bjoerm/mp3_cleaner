...
class TagBeautifier:
    ...
    def _beautify_strings(tags: list) -> list:
        ...
        output = [{k: StringBeautifier.beautify_string(v) if k in ["TALB", "TIT2"] else v for (k, v) in output[i].items()} for i in range(len(output))]  # Beautifying the album and song title.

        # Beautifying the artist. Here any leading "The" is cut.
        output = [{k: StringBeautifier.beautify_string(v, remove_leading_the=True) if k in ["TPE1", "TPE2"] else v for (k, v) in output[i].items()} for i in range(len(output))]
    ...
    def _check_obsolescence_of_album_artist(tags: list) -> list:
        ...
        if all(x is not None for x in album_artist) and all(x is None for x in track_artist):
            ...
            output = [{k: output[i].get("TPE2") if k in ["TPE1"] else v for (k, v) in output[i].items()} for i in range(len(output))]
        ...
        for i in output:
            if "TPE1" in i and "TIT2" in i:  # Only execute the following, if these two tags exists.
                (
                    ...
                    tpe1_updated,
                    tit2_updated,
                ) = StringHelper.move_feature_from_artist_to_track(artist=i["TPE1"], track_name=i["TIT2"])
    ...
    def _beautify_track_number(tags: list) -> list:
        ...
        helper_length_max = [NumberBeautifier.extract_number_from_slash_format(output[i].get("TRCK")) for i in range(len(output))]
    ...
    def _beautify_disc_number(tags: list, path: str = None) -> list:
        ...
        if any(x is None for x in is_each_track_with_number):
            ...
            [output[i].update({"TPOS": NumberBeautifier.extract_number_from_slash_format(output[i].get("TPOS"))}) for i in range(len(output))]
    ...
    def _beautify_date(tags: list) -> list:
        ...
        output = [{k: DateBeautifier.extract_year(v) if k in ["TDRC"] else v for (k, v) in output[i].items()} for i in range(len(output))]  # Shorten to YYYY.
