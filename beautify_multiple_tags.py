...
class TagBeautifier:
    ...
    @classmethod
    def beautify_tags(
        cls, tags: pd.Series, path: str, suffix_keywords: list
    ) -> pd.Series:
        """
        ...
        tags = (
            tags.to_list()
        )  # The following methods use list comprehensions which would have converted the pd.Series into a list anyway. This explicit step makes that more transparant.
        ...
        assert len(tags) == len(
            untouched_tag
        ), "Test for not loosing tags while beautifying."
    ...
    def _beautify_strings(tags: list) -> list:
        ...
        output = [
            {
                k: StringBeautifier.beautify_string(v) if k in ["TALB", "TIT2"] else v
                for (k, v) in output[i].items()
            }  # Beautifying the album and song title.
        ...
        output = [
            {
                k: StringBeautifier.beautify_string(v, remove_leading_the=True)
                if k in ["TPE1", "TPE2"]
                else v
                for (k, v) in output[i].items()
            }
    ...
    def _check_obsolescence_of_album_artist(tags: list) -> list:
        ...
        track_artist = [
            output[i].get("TPE1") for i in range(len(output))
        ]  # This is a list with an entry for each file/tag.
        ...
        if len(track_artist) == len(output):
            [
                output[i].pop("TPE2", None) for i in range(len(output))
            ]  # Remove the TPE2 tag.

        # Move album artist to track artist, if track artist is not available.
        if all(x is not None for x in album_artist) and all(
            x is None for x in track_artist
        ):
            # Copy tag from TPE2 to TPE1
            output = [
                {
                    k: output[i].get("TPE2") if k in ["TPE1"] else v
                    for (k, v) in output[i].items()
                }
                for i in range(len(output))
            ]
    ...
    def _check_feat_in_artist(tags: list) -> list:
        ...
        for i in output:
            if (
                "TPE1" in i and "TIT2" in i
            ):  # Only execute the following, if these two tags exists.
                (
                    has_feat_in_tpe1,
                    tpe1_updated,
                    tit2_updated,
                ) = StringHelper.move_feature_from_artist_to_track(
                    artist=i["TPE1"], track_name=i["TIT2"]
                )
    ...
    def _sort_track_name_suffixes(tags: list, suffix_keywords: list) -> list:
        ...
        output = [
            {
                k: StringHelper.sort_track_name_suffixes(
                    track_name=v, suffix_keywords=suffix_keywords
                )
                if k == "TIT2"
                else v
                for (k, v) in output[i].items()
            }  # Beautifying the title.
            for i in range(len(output))
        ]
    ...
    def _beautify_track_number(tags: list) -> list:
        ...
        helper_length_max = [
            NumberBeautifier.extract_number_from_slash_format(output[i].get("TRCK"))
            for i in range(len(output))
        ]

        helper_length_max = list(
            filter(None.__ne__, helper_length_max)
        )  # Removing all None values. From: https://www.kite.com/python/answers/how-to-remove-none-from-a-list-in-python

        # Edge case of all values being None values.
        if len(helper_length_max) == 0:
            return output  # End method. Don't do any beautification.

        helper_length_max = max(
            helper_length_max
        )  # Checking for the highest track number. Works also if multiple discs are present in the same folder.
        helper_length_max = len(
            str(helper_length_max)
        )  # Converting into the number if digits.

        output = [
            {
                k: NumberBeautifier.beautify_track_number(
                    v, helper_length_max=helper_length_max, minimum_length=2
                )
                if k in ["TRCK"]
                else v
                for (k, v) in output[i].items()
    ...
    def _beautify_disc_number(tags: list, path: str = None) -> list:
        ...
        helper_different_disc_number = set(
            output[i].get("TPOS") for i in range(len(output))
        )  # Set comprehension to remove duplciates.
        helper_different_disc_number = list(helper_different_disc_number)

        # Keeping or removing the disc number tag.
        if helper_different_disc_number is None or helper_different_disc_number == [
            None
        ]:  # There is no disc number tag.
            pass

        elif (
            len(helper_different_disc_number) > 1
        ):  # There are multiple different disc numbers in the same folder.
            pass

        elif (
            len(helper_different_disc_number) == 1
            and helper_different_disc_number[0] == "1/1"
        ):  # If there is only one disc number and that is disc number 1/1, remove the disc number tag.
            [output[i].pop("TPOS", None) for i in range(len(output))]

        elif (
            len(helper_different_disc_number) == 1
            and helper_different_disc_number[0] != "1"
        ):  # If there is only one disc number and that is not disc number 1, leave it.
            pass

        elif (
            len(helper_different_disc_number) == 1
            and helper_different_disc_number[0] == "1"
        ):  # If there is only one disc number and that is disc number 1, leave it.

            helper_folder_contains_cd_string = (
                NumberBeautifier.has_cd_string_in_folder_name(path)
            )

            if (
                helper_folder_contains_cd_string is True
            ):  # If there is a " cd" string in the folder name, don't remove the disc number tag.
                pass

            if (
                helper_folder_contains_cd_string is False
            ):  # If there is not a " cd" string in the folder name, remove the disc number tag.
                [output[i].pop("TPOS", None) for i in range(len(output))]
        ...
        if any(x is None for x in is_each_track_with_number):
            ...
            [
                output[i].update(
                    {
                        "TPOS": NumberBeautifier.extract_number_from_slash_format(
                            output[i].get("TPOS")
                        )
                    }
                )
    ...
    def _beautify_date(tags: list) -> list:
        ...
        output = [
            {
                k: DateBeautifier.extract_year(v) if k in ["TDRC"] else v
                for (k, v) in output[i].items()
            }  # Shorten to YYYY.
            for i in range(len(output))
        ]
