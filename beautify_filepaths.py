...
class FileBeautifier:
    ...
    def beautify_filenames(cls, df: pd.DataFrame, has_file_without_tags: bool):
        ...
        is_same_artist = cls._check_uniqueness_of_tag(
            tags=df.beautified_tag, id3_field="TPE1"
        )
        (
        ...
        is_same_disc_number = cls._check_uniqueness_of_tag(
            tags=df.beautified_tag, id3_field="TPOS"
        )
        is_same_album_title = cls._check_uniqueness_of_tag(
            tags=df.beautified_tag, id3_field="TALB"
        )
        is_same_date = cls._check_uniqueness_of_tag(
            tags=df.beautified_tag, id3_field="TDRC"
        )

        df[
            "beautified_filename"
        ] = cls._propose_multiple_filenames_from_multiple_tags(  # Filename only.
        ...
        df["beautified_filepath"] = cls._generate_beautified_pathlib_filepath(
            folder=df.folder, filename=df.beautified_filename
        )  # Folder plus filename.

        cls._write_filename(
            filepath_current=df.filepath, filepath_beautified=df.beautified_filepath
        )
        ...
        cls._rename_folder(
            folder_current=df.folder, beautified_folder=df.beautified_folder
        )
    ...
    def _check_uniqueness_of_tag(tags: pd.Series, id3_field: str) -> bool:
        ...
        unique_entries = set(
            tags[i].get(id3_field) for i in range(len(tags))
        )  # Using set comprehension instead of list comprehension to remove duplicates.
        ...
        if unique_entries is None or unique_entries == [None]:
            ...
            raise ValueError(
                "This should not be reached. Length of tag entries might be below 1."
            )
    ...
    def _check_existance_of_disc_and_track_number(cls, tags: pd.Series) -> tuple:
        ...
        is_each_track_with_disc_number = cls._helper_existance_of_disc_and_track_number(
            tags=tags, list_of_numbers=disc_numbers
        )
        is_each_track_with_track_number = (
            cls._helper_existance_of_disc_and_track_number(
                tags=tags, list_of_numbers=track_numbers
            )
        )
    ...
    @staticmethod
    def _helper_existance_of_disc_and_track_number(
        tags: pd.Series, list_of_numbers: list
    ) -> bool:
        """
    ...
    def _propose_single_filename_from_single_tag(
        ...
        artist = cls._beautify_string_from_tag(
            tag=tag.get("TPE1"), add_square_brackets=True
        )
        ...
        if (
            is_each_track_with_disc_number
            is True & is_each_track_with_track_number
            is True
        ):
        ...
        if is_each_track_with_track_number is True:
            number = number + cls._beautify_string_from_tag(
                tag=tag.get("TRCK")
            )  # Adding disc number, if it is present.
    ...
    def _beautify_string_from_tag(tag: str, add_square_brackets: bool = False) -> str:
        ...
        if tag is None or tag == "" or tag == " ":
            ...
            tag = str(
                tag
            )  # Short ensurer to prevent odd cases, when a tag is not a string.
    ...
    @staticmethod
    def _generate_beautified_pathlib_filepath(
        folder: pd.Series, filename: pd.Series
    ) -> list:
        """
        ...
        filepath = [
            folder[i] / filename[i] if filename[i] is not None else None
            for i in range(len(folder))
        ]  # Will write None if the filename also is None.
    ...
    def _write_filename(filepath_current: pd.Series, filepath_beautified: pd.Series):
        ...
        if check_for_none is True:
            ...
            check_filepath_already_exists = [
                pathlib.Path(filepath_beautified[i]).is_file()
                for i in range(len(filepath_beautified))
            ]
            ...
            [
                filepath_current[i].rename(filepath_beautified[i])
                for i in range(len(filepath_beautified))
            ]
    ...
    def _beautify_folder_name(
        ...
        if (
            has_file_without_tags is True
        ):  # If there are any mp3 files without tags in the folder, do not rename the folder.
            return
        ...
        if is_same_album_title is not True:
            ...
            (is_same_artist is False or is_same_artist is None)
            and not tags[0].get("TALB").endswith(("(Score)", "(Soundtrack)"))
            and is_same_album_title is not True
        ):
            return

        # Creating required single pieces.
        artist = cls._beautify_string_from_tag(
            tag=tags[0].get("TPE1"), add_square_brackets=True
        )  # Pulling the artist information from the information on the first mp3 file.
        ...
        if album.endswith(("(Score)", "(Soundtrack)")):
            artist = cls._beautify_string_from_tag(
                tag=tags[0].get("TALB"), add_square_brackets=False
            )
            artist = regex.sub(r"\(.+\)", "", artist)  # Remove any strings in brackets.
            artist = artist.strip()
            artist = regex.sub(
                r"\s\p{Pd}\s.+$", "", artist
            )  # Remove anything that starts with a space-hyphen-space pattern.
            artist = artist.strip()
            artist = regex.sub(
                r"\s\d+$", "", artist
            )  # Remove any integers add the end. If this was Terminator 2, this will be changed into Terminator.
            artist = artist.strip()
            artist = f"[{artist}]"
        ...
        if (
            (is_same_artist is False or is_same_artist is None)
            and not tags[0].get("TALB").endswith(("(Score)", "(Soundtrack)"))
            and is_same_album_title is True
        ):
        ...
        if is_each_track_with_disc_number is True:
            disc_number = cls._beautify_string_from_tag(
                tag=tags[0].get("TPOS")
            )  # This just takes the disc number from the first file.
        ...
        if (
            is_same_date is True
            and is_each_track_with_disc_number is True
            and is_same_disc_number is True
        ):
            ...
            is_same_date is True
            and is_each_track_with_disc_number is True
            and is_same_disc_number is False
        ):
            beautified_folder = f"{artist} - {album} ({date})"

        elif is_same_date is True and (
            is_each_track_with_disc_number is False
            or is_each_track_with_disc_number is None
        ):
            beautified_folder = f"{artist} - {album} ({date})"

        elif (
            (is_same_date is False or is_same_date is None)
            and is_each_track_with_disc_number is True
            and is_same_disc_number is True
        ):
        ...
        elif (is_same_date is False or is_same_date is None) and (
            is_each_track_with_disc_number is False
            or is_each_track_with_disc_number is None
        ):
            beautified_folder = f"{artist} - {album}"
    ...
    def _rename_folder(folder_current: pd.Series, beautified_folder: pd.Series):
        ...
        if check_for_none is True:
            ...
            beautified_folder = (
                folder_current.parent / beautified_folder
            )  # Only rename the last item of the folder path. Files in subdirectories are that way not moved around.
